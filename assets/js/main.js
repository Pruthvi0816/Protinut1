(function ($) {
	...
	// Use document.ready to hide the preloader as soon as the DOM is parsed,
	// rather than waiting for window.onload which can hang on slow image/font requests.
	...
	$('.xb-menu-toggle').on('click', function (e) {
		e.stopPropagation();
		e.preventDefault();
	...
	$('.xb-nav-hidden li.menu-item-has-children > a, .xb-menu-primary li.menu-item-has-children > a').click(function (e) {
		var target = $(e.target);
		if ($(this).attr('href') === '#' || $(this).attr('href') === 'javascript:void(0);') {
			e.preventDefault();
			e.stopPropagation();
			var toggleBtn = $(this).find('.xb-menu-toggle');
	...
	$(document).ready(function () {
		$(document).on('submit', '#contactForm', function (e) {
			e.preventDefault();
			var form = $(this);
			var feedback = form.find('#contactFeedback');
			if (feedback.length === 0) {
				feedback = form.parent().find('#contactFeedback');
			}
			if (feedback.length === 0) {
			...
			fetch('contact_action.php', {
				...
				.then(res => {
					if (res.success) {
						feedback.html('<span style="color:#4caf50;font-weight:bold;">' + res.message + '</span>');
						feedback.show();
						form[0].reset();
					} else {
						feedback.html('<span style="color:#f44336;font-weight:bold;">' + res.message + '</span>');
						feedback.show();
					}
				})
				.catch(error => {
					feedback.html('<span style="color:#f44336;">Something went wrong. Please try again.</span>');
					feedback.show();
				});
		});
	});
